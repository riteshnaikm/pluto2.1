...
3. Show average time to generate handbook & evaluation on Dashboard (verify if already implemented).
4. Reduce generation time under 20 seconds (resume eval + handbook).
5. Restrict to **one handbook per job ID** — if a handbook exists, show it instead of creating a duplicate.
